...
8. **切换到 443 已完成（2026-09-16）。** 执行顺序即此前记录的那一条：`systemctl disable --now xray` → `POST /panel/api/inbounds/update/<id>` 把端口从 44300 改 443。
   ...
   **尚未记录的复核项**（不要当已完成）：`systemctl is-enabled xray` 的实际输出（必须是 `disabled`，否则下次重启两个 unit 抢 443）、切换后 privateKey 的 sha256 前 16 位复核结果（基准 `2a876c1641027973`）、切换后的计数器读数与实际停机时长。这几项当时没有回填。
...
- ~~机器 A 切换到 443~~ **已完成（2026-09-16）**，见「机器 A 迁移进度」第 8 条。**遗留：回填该条列出的三项复核**（`systemctl is-enabled xray`、privateKey 哈希、计数器与停机时长），以及 v2rayNG 的版本号与其打包的 Xray 核心版本。
