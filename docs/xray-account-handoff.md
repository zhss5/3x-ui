...
2026-09-18 状态更新（两件大事）：**(一) 代码基线合并到上游 v3.8.5**（合并 `881d5d18`，基线说明见「最新仓库与架构决定」；机器 A 仍跑 v3.7.0 发行二进制，刻意不升级）。**(二) S3 的连接控制方向已确定并实测通过**——`RemoveUser` + `SOCK_DESTROY`，不重启核心即可断开超额用户、新周期热加回，见「S3 连接控制方向已确定（2026-09-18）」。S3 自此解除阻塞。同日还在 v3.8.5 上重新核对了 S0/S1 的全部结论（见「v3.8.5 上的 S0/S1 复核结果」），并在本机/WSL 建起了完整开发与验证环境。

2026-09-18 当日续（三件）：**(三) 两个地址族的 socket 销毁均已实测打通**，其中真 IPv6 对端需要自建 AF_INET6 请求，现成库对它无效——见「netlink 库的地址族边界」。**(四) 实现计划已产出**：[S3 定向断连实现计划](superpowers/plans/2026-09-18-s3-targeted-disconnect.md)，8 个任务 46 步，含机器 A 端到端验收。**(五) 部署决定待用户拍板**，见「部署自建面板的待决事项（2026-09-18）」——这是代码之外唯一挡在验收前面的东西。
...
因此「额度耗尽即断流」在 v3.7.0 的**面板既有能力**上做不到。当时列出三个候选方向且均未验证：

1. 接受可测量的超用窗口——只挡新连接，配合轮询与明示的延迟上界；
2. 在连接层扩展准入校验，使既有连接可被定向终止；
3. 整核心重启——代价是同机所有账号掉线。

**2026-09-18 已选定方向并实测通过：方向 2 的一个变体——`RemoveUser` 挡新握手 + 内核 `SOCK_DESTROY` 定向销毁该用户既有连接，全程不重启核心，且无需修改 xray-core。** 详见「S3 连接控制方向已确定（2026-09-18）」。上文第 3 条原写作「本文件已明确禁止」，现修正为记录代价：上游 v3.8.5 恰恰把它做成了默认行为（`e790f467`），本项目选择不采用它，理由同见该节。
...
**取证方法**：核心计数器与数据库累计的差额能反推进程重启。核心计数器从不清零（`GetTraffic` 用 `Reset_: false` + 差分基线），所以它只可能小于数据库累计，且差额就是历次重启前积累的量。实测 `user1` 核心侧 51.5 MiB、数据库侧 314.55 MiB，差 263 MiB，说明中途重启过。再用 `ps -o lstart,etime` 与面板日志定位那次重启。

**重启日志全表**（`/var/log/x-ui/3xui.log`，全部四条）：

| 时刻 | force | 归因 |
| --- | --- | --- |
| 2026-09-15 09:50:35 / 10:13:56 / 10:30:22 | `true` | 面板初装与加固 |
| 2026-09-17 10:55:29 | `true` | 加 systemd 加固 drop-in 所需的 `systemctl restart x-ui` |

**`force: true` 是关键判据。** 它对应 `RestartXray(true)`，而崩溃检测（`check_xray_running_job.go:33`）与 pending-restart（`xray.go:1514`）走的都是 `RestartXray(false)`——**正是客户端改动热更失败会触发的那两条**。四次全是 `true`，在本机配置下（零节点、无 TG bot、未换过 Xray 版本）唯一可能的调用方是面板启动路径 `web.go:314`。
...
**263 MiB 差额至此完全解释**，与改名无关：当前进程 2026-09-17 10:55:29 才启动，那 263 MiB 是上一个进程（09-15 10:30 → 09-17 10:55）积累的。

**副产品发现：`systemctl restart x-ui` 是一次全量断线。** `x-ui.service` 没有 `KillMode=` 指令，用 systemd 默认的 `control-group`，重启面板会连带杀掉作为子进程的 Xray。所以当日 10:55 加 systemd 加固时，**所有用户断过一次**。凡需重启面板的操作——加固、升级、改环境变量——都是全量断线，应按变更窗口对待。
...
## S3 连接控制方向已确定（2026-09-18）

**决定：`RemoveUser` 挡新握手 + 内核 `SOCK_DESTROY` 定向销毁该用户的既有连接。全程不重启 Xray 核心，且不需要修改 xray-core。** 这解除了 S3（共享月额度）自 2026-09-14 起的唯一阻塞项。

### 为什么不用上游 v3.8.5 的做法

上游提交 `e790f467`（PR #6550，2026-09-15）把「整核心重启」做成了默认断流手段：`restartXrayOnClientDisable` 默认 `"true"`，停用、删除、批量停用、以及额度耗尽自动停用都会让核心在 30 秒内（自动路径一次 5 秒轮询内）整体重启。**该提交自己的说明写明：核心没有关闭单个会话的 API。** 本项目不采用它，两个理由：

1. **代价**：核心重启断掉该机器上所有人，一个账户超额会牵连全部账户。
2. **它本身不可靠**（复核实测确认）：`restartToDropClients` 守卫只在「被移除的客户端仍存在于运行进程的配置快照里」时才触发，而快照只由 `NewProcess` 与 `tryHotApply` 里的 `SetConfig` 写入；**经运行时 API 热加回的客户端不会进入快照**。于是月初重置热加回之后，下一次耗尽不再触发重启，连接照旧存活。批量删除、批量解绑、入站级停用、以及 `UpdateInbound` 路径同样不触发。

因此本项目把 `restartXrayOnClientDisable` 关掉，改用下述机制。

### 机制

```
额度耗尽:  数据库 enable=false（并记录停用原因）
           → RemoveUser（挡住新握手）
           → 查该 email 当前在线 IP → 枚举本机 TCP socket → SOCK_DESTROY 销毁
新周期:    数据库清零已用流量、enable=true → AddUser（热加回，放行新握手）
```

顺序固定为**先改数据库、再动核心**：核心那一步失败时数据库仍是对的，下次核心因任何原因重启都会按数据库生成正确配置；反之则会在重启后把用户又踢出去。
...
### 实测记录（机器 A，2026-09-18）

脚本：[`superpowers/validation/testdata/s3-sockdestroy/killprobe.sh`](superpowers/validation/testdata/s3-sockdestroy/killprobe.sh)。隔离方式：机器上现成的 26.7.28 二进制另起两个独立进程，只监听 `127.0.0.1` 的 44310-44313，一次性生成的 REALITY 密钥，销毁前用同一过滤条件先列出并硬校验再人工确认。

| 判据 | 结果 |
| --- | --- |
| 内层 TLS（splice 前提） | `XtlsFilterTls found tls 1.3` |
| 本次下载确实进入 splice | 是（新增 1 次 `CopyRawConn splice`） |
| `RemoveUser` 后已有连接继续传输 | **是，8 秒 +10,092,544 字节** |
| **销毁 socket 后传输停止** | **是——连续 5 秒全部 +0 字节，curl 退出，端口上剩余 established 连接 0 条** |
| 出站侧残留连接 | **0 条** |
| 移除后新连接被拒 | 是（HTTP 000） |
| `AddUser` 后重连成功且核心未重启 | 是（HTTP 200，pid 全程未变） |
| 生产 443 未受影响 | 是（pid 84559 未变，连接 4 → 4） |

服务端日志给出了确切机制，销毁后立刻出现：

```
proxy/freedom: connection ends > read tcp 127.0.0.1:44310->127.0.0.1:41038:
read: software caused connection abort
```

即内核中止 socket → splice 的 `ReadFrom` 报错 → freedom 出站结束 → 两端一起拆除。**这同时否掉了一个此前的担忧**：原以为出站侧可能残留文件描述符直到对端发数据或 splice 的 24 小时空闲超时，实测残留 0 条，不存在 fd 泄漏。
...
| 同上，改用自建 AF_INET6 请求 | AF_INET6 | `<nil>` | **已销毁**（服务端写入立即 `connection abort`） |
...
2. **真 IPv6 客户端是库的硬缺口，但可以绕过，且已实测绕通。** 库用 `To4() == nil` 拒绝非 v4 地址并返回 `not implemented`，而内核本身完全支持。**若只用现成库，IPv6 客户端永远断不掉**——这是静默的配额漏洞，不是可接受的边角：用户能否走 IPv6 由客户端侧决定，面板无从控制。

   拦路的只是 `SocketDestroy` 的那道闸：库内部的 `socketRequest.Serialize()`（`socket_linux.go:46-66`）**本就正确处理 `AF_INET6`**。因此不需要改库或调外部命令，自己按同一线格式（56 字节）拼一个 `Family = AF_INET6` 的 `SOCK_DESTROY` 请求，用 `nl` 子包的导出 API 发出去即可——`nl.NewNetlinkRequest(nl.SOCK_DESTROY, unix.NLM_F_ACK)` + `req.Execute(unix.NETLINK_INET_DIAG, 0)`。**已在机器 A 上实测：返回 `<nil>`，四元组消失，服务端下一次写入立刻 `software caused connection abort`。** 可用代码见归档的 `sockprobe.go.txt` 中的 `socketRequest6` / `destroyV6`。

   于是实现只需按对端地址二选一：`To4() != nil` 走库，否则走自建请求。全程不依赖 `ss`。
...
2. **同出口 IP 的连带影响没测。** 在线表只记到 IP、不记端口，因此只能按（本地端口, 对端 IP）销毁；同一公网 IP 下的其他用户会被连带断开一次，他们仍有权限、客户端会自动重连。可以检测到这种情况（同一 IP 挂在多个 email 下）并记日志。

   **2026-09-18 用户决定：第一版就按这个做，不追求按用户精确断连。** 根因是 `OnlineIP` 只有 `IP` 与 `LastSeen` 两个字段，没有源端口；而 NAT（家庭路由、公司网络、运营商 CGNAT）后面的多个人对外是同一个 IP，只有源端口能区分他们。所以即便 email → IP 查得完全准确，也只能做到「杀掉来自这个 IP 的所有连接」。要做到精确需要 email → (IP, 端口) 的连接级映射，当时评估过两条路并**明确暂不采用**：
   - **开 Xray 访问日志**：它逐条记 `from IP:端口 ... email:`，正好是所需映射；但会记录每个用户访问的每个目标地址，对代理服务是很重的隐私代价，只实时解析、丢弃目标地址、不落盘又实现不简单。
   - **改 Xray 核心加「按用户关闭会话」API**：最干净，但从此不再跑原版二进制，要自己维护 fork 并跟上游。

   采用按 IP 杀的理由：被连带的人**没有被 `RemoveUser`**，重连立即成功，体验是「网络抖一下」而非「用不了」；对比被弃用的整核心重启方案（每次让**所有人**抖一下），这已经好一个量级，而额度耗尽一个月可能就几次。

   **因此「检测并记日志」从「可以做」升级为必做项**：每次销毁前检查同一 IP 是否同时挂在多个 email 下，命中则记录（IP、被停用的 email、被连带的 email 列表）。这份日志用来回答一个只有部署后才能回答的问题——这批用户里到底有多少人共享出口 IP。几乎没有则维持现状；冲突频繁（例如大量用户在同一运营商 CGNAT 后面）再重新评估访问日志方案。

   **测试约束**：本机 WSL2 内核是 `# CONFIG_INET_DIAG_DESTROY is not set`，**`SOCK_DESTROY` 在 WSL 里测不了**，调用会返回 `EOPNOTSUPP`；机器 A 实测为 `=y`，是唯一可测之处。所以本条及第 1 条的实测必须在机器 A 上做，并沿用本节的隔离做法——独立 Xray 进程、只监听 `127.0.0.1` 临时端口、一次性 REALITY 密钥，不碰 443、面板与 `x-ui.service`。
3. **多节点传播没测。** 断连必须发生在连接落地的那台机器上。
...
- **部署后果**：这些改动在本仓库，要生效就得自己编译面板替换机器 A 上的上游 v3.7.0 二进制，即第三条路（v3.8.5 面板 + 钉住 26.7.28 核心）正式上生产，且机器 A 从此跑自建构建。回退流程需单独设计。

## v3.8.5 上的 S0/S1 复核结果（2026-09-18）

合并 v3.8.5 后，本文件 2026-09-18 之前记录的行号全部指向 v3.7.0，故对四个维度做了重新核对，每条结论另配一个对抗复核者。要点如下。

**流量账本（S1 实验 1）：上游没修，缺陷全部仍在。** 把 S1 的探针原样拷回 `internal/web/service` 在合并后的代码上重跑，**再次输出 `DELTA LOST`**；去掉注入故障的对照组则通过，证明丢数据确由写库失败引起。代码只是位移：`inbound_traffic.go:184-194`（原 178-188）、`api.go:772/785`（原 748/761）。另两点值得记：PostgreSQL 上真实的 UPDATE 失败会让整个事务回滚，损失更大但不再静默；**每次核心重启还会额外丢掉最多一次 5 秒轮询内所有人的流量**（`RestartXray` 在 `process.Stop()` 前不做最后一次 `GetTraffic`），而 v3.8.5 重启更频繁，两者叠加。误差方向单一——账本只会少算，所以额度控制是「失效时放行」。

**断流能力（S1 实验 2a）：`RemoveUser` 仍然断不了既有连接**（本日实测再次确认，且是在 splice 场景下）。`check_client_ip_job.go` 的 IP 超限「断开所有连接」仍然名不副实。`e790f467` 改变了停用后的行为，但如上节所述既有代价又不可靠。

**S0 基线：结论依然成立。** S0 记录里点名的 11 个测试全部仍存在且通过（`make test-go` 在合并后的树上 45 包全绿），跨节点按 `email` 汇总的机制与 v3.7.0 相同，`client_traffics.email` 仍是共享累加器。两个新约束：

- 安全修复 `a31fa9ab` 使**从节点快照认领来的客户端不再链接到第二个节点**，master 执行额度耗尽时只会停掉已链接的那台，另一台照常服务。本项目「管理员在 master 开户」的设计不受影响，但 S2/S3 必须加一道「每个账户在每个节点都有链接」的检查。该状态可由 master 侧任一次 `SyncInbound`（管理员保存入站等）修复。
- **每次节点同步都会用该节点的值覆盖 master 上的 `client_traffics.total`**（自 v3.7.0 起如此）。共享额度用的是最后一个同步上来的节点报的数，除非各节点的副本一致。

**客户端操作注意事项：一条已修、其余仍在。** `5fbd2b49` 修了「漏传 `enable` 造成 `clients` 与 `client_traffics` 两张表不一致」——现在显式传 `enable:false` 会被尊重（v3.7.0 会被强制改成 true）。仍然存在的：`compactOrphans` 静默剔除孤儿、`/clients/update/:email` 与 `/inbounds/update/:id` 均为全量替换且不保留 `flow`/`enable`、`inboundCanEnableTlsFlow` 不认 xray 新名 `raw`、email 大小写导致身份分裂（v3.8.5 没有加不区分大小写的唯一索引）。**新发现一个坑**：`x-ui setting -getApiToken -tokenName X` 走 `RecreateByName`，会把任何同名 token 重建成**永不过期的 admin**——拿它轮换一个 monitor 或 node-sync token 会悄悄提权。

## 部署自建面板的待决事项（2026-09-18）

代码之外唯一挡在 S3 验收前面的东西。[实现计划](superpowers/plans/2026-09-18-s3-targeted-disconnect.md)的改动都在本仓库，不把自建面板放到机器 A 上就一行都不会生效。

### 机器 A 当日实测形态（不要凭记忆，这里有过一次判断错误）

```
84551  ppid=1      /usr/local/x-ui/x-ui              上游 3x-ui v3.7.0 发行二进制，70 MB，Aug 24
84559  ppid=84551  bin/xray-linux-amd64              xray-core 26.7.28，监听 *:443（当时 3 条 established）
```

**机器上只有一个 xray，而且它是面板的子进程。** 编写实现计划时曾写成「另有一个独立的老版 xray 跑在 443 服务老版 v2rayN 用户，不得触碰」，**这是错的**，已于 `b02f89c9` 更正。2026-09-16 那次「把面板的核心切到 443、裸 Xray 26.2.6 `disable --now`」早已完成，裸核心不再存在。

这个更正带出一个原先被掩盖的后果：**替换面板二进制必须重启 x-ui，而重启 x-ui 会连带重启它的子进程 xray，即当场断掉 443 上的所有人。** 这正是整个 S3 要消灭的那种全员断线，作为部署的一次性代价躲不掉——必须挑低峰、事先告知、事后立即确认所有人能重连。

### 版本线必须分清（易混）

| | 机器 A 现在 | 部署后 |
| --- | --- | --- |
| 面板 3x-ui | v3.7.0 | **v3.8.5**（本分支自行编译） |
| 核心 xray-core | 26.7.28 | **26.7.28，不动** |
...
本机已能免密 SSH 直连机器 A（`root@38.59.228.104`，ed25519 密钥，22 端口），命令输出原路返回，不需要隧道——此前会掉线的隧道是给面板 Web 界面做端口转发用的。这改变了此前「我给命令、用户在节点上粘贴执行」的协作方式，迭代快得多。**但边界不变**：只做只读查看，或使用探针自己的临时端口与自己的连接；不碰 443、面板配置与任何生产数据；一次性文件用完即删。涉及生产变更（如本节的部署）仍须先经用户同意。

## 本机与 WSL 开发验证环境（2026-09-18 建成）

此前记录「本机没有 Go 工具链」已过时。当前实际状况：

- **Windows 侧**：Go **1.27.1**（装在 `D:\Program Files\Go`，系统级 PATH；v3.8.5 的 `go.mod` 正好要求 1.27.1）+ gcc 16.1.0（MSYS2，CGO 编 go-sqlite3 实测通过）。注意 PATH 上还有 Codex 装的 go 1.26.6，靠工具链自动切换才到 1.27.1——装了 1.27.1 之后不再依赖该机制。
- **WSL（Ubuntu 24.04，与机器 A 同版本）**：go 自动切换到 1.27.1、gcc 13.3、make 4.3、Node **24.21.0**（官方 tarball + SHA256 校验）、golangci-lint **2.13.2（用 go1.27.1 编译，与项目 Go 版本对齐）**、Xray **26.7.28**（官方 `.dgst` 校验，与机器 A 同一构建 `5ca6f4b`）。环境脚本 `~/xui-dev/env.sh` 设好 `XUI_DB_FOLDER`/`XUI_BIN_FOLDER`/`XUI_LOG_FOLDER`，本地面板**不需要 root** 即可跑。
- **`make verify` 除 `test-fe` 外全部通过**（gen-check / lint-go / lint-fe / format-check / typecheck / msw-worker-check / build / build-storybook，加上先前的 test-go 45 包全绿）。`test-fe` 需要 Playwright 的 chromium 与系统库（要 sudo），尚未安装。
- **第三条路首次获得实测支持**：本地跑 **v3.8.5 面板 + Xray 26.7.28 核心**，核心正常启动并载入生成的配置、无重启循环，面板还识别到该版本支持 online-stats API。注意这是空业务入站的情况；带 VLESS+REALITY 入站的完整验证尚未做。
- **WSL 内核不支持 `SOCK_DESTROY`**（`# CONFIG_INET_DIAG_DESTROY is not set`），所以断连一类的实验只能在机器 A 或其他标准内核机器上做；但脚本的其余机制（配置生成、握手、splice 判定、socket 枚举与过滤、`rmu`/`adu`）都已在 WSL 里用 `DRYRUN=1` 验证过。
...
- **master 宕机期间允许用户多用。** 每个账户最坏多用 (N−1) 份额度，两节点即一整份；不为「master 失联」另建机制。
- **删除整个入站时不断流，第一版不加挂钩。** 会删整个入站的只有两类情况：
  - 管理员删入站，以及它在节点恢复后的对账清理。受影响的是仍有额度的正常用户，残留连接跑到空闲超时为止；需要立即断开时，手工在节点上重启一次 Xray。
  - 入站级总流量或到期。**部署约定：入站不设总流量与到期时间**，这种情况就不会发生。
...
3. **进行中。** 记账可靠性与定向断流已在单机真实核心上验证完毕（实验 1、2a，见上节「已验证的执行能力事实」）。剩余：实验 3（节点失联收敛，需第二个节点）、UDP / Mux / REALITY / flow 场景、目标 Linux 节点复跑。2026-09-11 记录的 [具体验证方法](superpowers/plans/2026-09-11-xray-account-baseline-validation.md#双节点与定向断流验证方法讨论补充) 仍适用于未覆盖部分。
   ~~**新增阻塞项**：连接控制方向（超用窗口 / 连接层扩展 / 其他）必须先定下来，否则 S3 无法开工。~~ **2026-09-18 已解除**：方向定为 `RemoveUser` + `SOCK_DESTROY` 并在机器 A 实测通过，见「S3 连接控制方向已确定（2026-09-18）」。
...
- ~~S3 共享额度实现 → 阻塞于连接控制方向未定~~ **2026-09-18 解除阻塞，可以开工**。方向已定并实测通过。开工前需按「实现要点」一并处理：挂钩位置（不能挂裸 `RemoveUser`）、新增停用原因字段（含数据库迁移）、关闭 `restartXrayOnClientDisable`、`AddUser` 失败改重试、修流量账本丢增量缺陷。实现阶段还需补三项未覆盖的实测：真实远端客户端上的 email→IP 查询、同出口 IP 的连带影响、多节点传播。
...
- **`x-ui.service` 已加 systemd 加固 drop-in（2026-09-17 完成）。** 加固前实测该 unit 除 `ExecStart=/usr/local/x-ui/x-ui` 外**没有任何指令**——无 `User=`、无 capability 限制、无 `Protect*`，即以完全无约束的 root 运行。关键在于 **xray 是它的子进程、在同一 service cgroup 里，因此继承该 unit 的沙箱**，所以这等于「唯一暴露在公网 443 的进程当时毫无约束」。
  ...
  **诚实定位与边界（务必读完再评估价值）**：uid 仍然是 **0**，这六条只是在 root 之上关掉了几条特定路径。**它们完全没有限制文件系统访问**——这是刻意排除 `ProtectSystem` 的直接后果。所以一个拿到该进程代码执行的攻击者依然能读写任意文件（含 `/etc/shadow`、SSH 私钥、以及 `x-ui.db` 里的 REALITY 私钥与全部客户端 UUID）、往 `authorized_keys` 加公钥、写 cron 或新 unit 做持久化、并看到所有过境流量（它本就是代理）。准确的定位是：**封掉了「内核级 rootkit」与「setuid 提权」两类路径，没有封掉「以 root 身份读写这台机器」**。封内核那类的成本是零、也确实是真实威胁，所以不算白做，但离「被打穿也无大碍」很远。回退一条命令：`rm -f /etc/systemd/system/x-ui.service.d/hardening.conf && systemctl daemon-reload && systemctl restart x-ui`。
...
- **待定的下一层加固：`ProtectSystem=strict` + `ReadWritePaths`。** 上一条列出的六条不限制文件系统，而真正能约束「被打穿后的损害范围」的正是这一条——它把整个文件系统变成只读、只放行白名单目录，从而挡住往 `authorized_keys` / cron / 新 unit 写入这类持久化。它是当日刻意从第一批里排除的（可动部件多、写错会打断数据库写入），但**可以作为独立一步追加**，同样走 drop-in。**前置条件：必须先实测确认 x-ui 究竟往哪些路径写，不能凭猜**——已知至少有 `/etc/x-ui`（DB 及 `backupSQLite` 在 `filepath.Dir(GetDBPath())` 下建的临时目录）、`/usr/local/x-ui`（`bin/config.json` 在真正重启时重写）、`/var/log/x-ui`；证书签发、geodata 更新等路径尚未核实。验收方式明确：加上之后跑一次面板写操作（例如改客户端备注）确认能落库，并复核 443 的 pid 与两个客户端的 `flow`。
