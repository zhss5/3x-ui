...
- **在 44300 上测 v2rayNG（最新版），并复测 Shadowrocket 2.2.92**（步骤见「客户端兼容性硬约束 → 切换前要做的实测」）。前置第一步是核实机器 A 面板核心的**运行中版本**，不要沿用记录。两者都通过则无需任何配置改动即可切 443。**这是切换决策的唯一依据。**
...
- 机器 A 切换到 443 → 阻塞于 **v2rayNG 未验收**（范围已于 2026-09-16 收窄，Clash/mihomo 暂不阻塞）。注意这不是「等门槛解除」：443 现在的裸 26.2.6 无闸，切换本身会给生产端口新增限制，所以必须先证明目标状态下必测清单全通，再切。
