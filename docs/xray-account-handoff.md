...
2026-09-16 当日三次更新（切换已解除阻塞）：收窄后的必测清单 **v2rayN / Shadowrocket 2.2.92 / v2rayNG 全部通过 44300**，且是在核心版本经实测确认为 26.7.28、默认门槛活跃的前提下通过的。**机器 A 切换到 443 不再被客户端兼容性阻塞，且不需要任何配置改动。** 剩余风险与切换步骤见「客户端兼容性硬约束 → 验收结果」与「机器 A 迁移进度」。
...
- ~~在 44300 上测 v2rayNG 并复测 Shadowrocket~~ **已完成（2026-09-16）**，连同核心版本核实一起。三个必测客户端全通过。**切换到 443 现在是可立即推进项**，步骤见「机器 A 迁移进度」第 7 条之后；执行前重读「验收结果」里列的三条剩余风险。
...
- ~~机器 A 切换到 443~~ **已解除阻塞（2026-09-16）**，移至上面「未被阻塞」。
