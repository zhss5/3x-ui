...
> **这一步会动生产，先读完再动手。** 执行前先跟用户确认这次部署，不要自行决定。
>
> 机器 A 的实际形态（2026-09-18 实测，不要凭想象）：
>
> ```
> 84551  ppid=1      /usr/local/x-ui/x-ui        ← 上游 v3.7.0 发行二进制
> 84559  ppid=84551  bin/xray-linux-amd64        ← 核心 26.7.28，占着 *:443
> ```
>
> **机器上只有一个 xray，而且它是面板的子进程。** 没有第二个「独立的老版 xray」——把 x-ui 的核心切到 443 那一步早已完成。这条推论必须跟着改：
>
> **替换面板二进制必然要重启 x-ui，而重启 x-ui 会连带重启它的子进程 xray，也就是当场断掉 443 上的所有人。** 这正是本计划要消灭的那种全员断线，只不过这次是部署的一次性代价，躲不掉。因此：挑低峰时段做，动手前告诉用户会有一次全员瞬断，做完立刻验证所有人能重连。
>
> 核心版本必须仍是 **26.7.28**：26.9.x 会切断 Shadowrocket，而 Shadowrocket 占在用客户端的一半。只替换 `/usr/local/x-ui/x-ui`，不要碰 `/usr/local/x-ui/bin/` 下的核心。
...
Expected: `active`，且核心版本仍是 **26.7.28**。版本不对立刻按 Step 3 回退。

随后**立刻确认所有人都能重连**——刚才的重启把 443 上的每个人都断了：

```bash
ssh root@38.59.228.104 'sleep 30; ss -tn state established "( sport = :443 )" | tail -n +2 | wc -l'
```

Expected: 连接数回到部署前的水平。回不来就按 Step 3 回退，不要带着问题往下走。
