...
| 顺序 | S1 必须新增的行为验证 | 决定什么 | 执行状态 |
| --- | --- | --- | --- |
| 1 | 读取统计后注入数据库提交失败，再次采集；节点重复/迟到快照；核心/节点重启与归属变更 | 持久化计量起点、去重键、计数器世代和异常可见性 | **部分完成 2026-09-14**：注入提交失败已复现增量永久丢失且无错误信号；重复/迟到快照与重启换归属未做 |
| 2 | 账户 A/B 先建立持续 TCP 流；停用 A 后新连接被拒、旧流停止，B 不受影响；再测 UDP、Mux、VLESS + REALITY/实际 flow | 现有运行时能否执行目标，是否需要 Xray 连接层扩展 | **部分完成 2026-09-14**：真实核心上 TCP 场景已测，`RemoveUser` 对既有连接零作用、B 不受影响、新握手被拒；UDP / Mux / REALITY / flow 未做 |
| 3 | 一节点失联时调额/停用，恢复后接收最新状态；重复执行及旧指令迟到 | 节点失败策略、重试和版本控制；没有证据不能显示已生效 | **未执行**：需要第二个节点 |

实测结果、证据与边界见 [S1 实验记录](../validation/2026-09-14-xray-account-s1-experiments.md)。

S1 的详细测试代码和连接控制方案根据 S0 结果另成计划；本计划不预设现有 API 会断开已有连接。若需要新增核心能力，先形成具体接口和行为方案，不以全核心重启或只隐藏订阅替代。

2026-09-14 补充：实验 2 已证明现有 API **不能**中断既有连接，因此「额度耗尽即断流」在现有能力上不成立，连接控制方案是 S3 的前置阻塞项。
